...
fpfhFeature::Ptr compute_fpfh_feature(
...
int main(int argc, char** argv) {
  ...
  pcl::transformPointCloud(*cloud, *source, transform_2);

  std::cout << "cloud size is: " << source->size() << std::endl;

  // 体素化
  // pcl::ApproximateVoxelGrid<pcl::PointXYZ> approximate_voxel_grid;
  pcl::VoxelGrid<pcl::PointXYZ> approximate_voxel_grid;
  approximate_voxel_grid.setLeafSize(
      1, 1, 1);  // 网格边长.这里的数值越大，则精简的越厉害（剩下的数据少）
  ...
  sac_ia.setNumberOfSamples(
      20);  // 设置每次迭代计算中使用的样本数量（可省）,可节省时间
  sac_ia.setCorrespondenceRandomness(
      10);  // 设置计算协方差时选择多少近邻点，该值越大，协防差越精确，但是计算效率越低.(可省)
  sac_ia.align(*align);

  std::cout << "计算结束" << std::endl << std::endl;
  cout << "代码段运行时间: " << time.toc() / 1000 << "s"
       << endl;  // 计算程序运行时间
  // 可视化
  ...
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>
      sources_cloud_color(source, 250, 0, 0);
  view.addPointCloud(source, sources_cloud_color, "sources_cloud_v1", v1);
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>
      target_cloud_color(cloud, 0, 250, 0);
  view.addPointCloud(cloud, target_cloud_color, "target_cloud_v1", v1);
  view.setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "sources_cloud_v1");

  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>
      aligend_cloud_color(align, 255, 0, 0);
  ...
  view.setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 4, "aligend_cloud_v2");
  view.setPointCloudRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "target_cloud_v2");
  ...
  view.addCorrespondences<pcl::PointXYZ>(source, cloud, *cru_correspondences,
                                         "correspond", v1);  // 添加显示对应点对

  view.setShapeRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 2,
      "correspond");  // 设置对应点连线的粗细.PCL_VISUALIZER_LINE_WIDTH,表示线操作,线段的宽度为2（提醒一下自己:
                      // 线段的宽度最好不要超过自定义的点的大小）,"correspond"表示对
                      // 对应的标签 做处理.
  view.setShapeRenderingProperties(
      pcl::visualization::PCL_VISUALIZER_COLOR, 0, 0, 1,
  ...
  cout << "代码段运行时间: " << time.toc() / 1000 << "s"
       << endl;  // 计算程序运行时间
  while (
      !view.wasStopped()) {  // Display the visualiser until 'q' key is pressed
    view.spinOnce();
  }
