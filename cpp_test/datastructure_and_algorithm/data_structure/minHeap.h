...
void minHeap<T>::pop() {  // Remove max element.
  ...
  while (child <= heapSize) {
    ...
    if (lastElement <= heap[child]) break;  // yes
...
void minHeap<T>::initialize(
    T* theHeap,
    int theSize) {  // Initialize max heap to element array theHeap[1:theSize].
  delete[] heap;
  heap = theHeap;
  heapSize = theSize;
  ...
  for (int root = heapSize / 2; root >= 1; root--) {
    ...
    while (child <= heapSize) {
      ...
      if (rootElement <= heap[child]) break;  // yes
...
void minHeap<T>::output(
    ostream& out) const {  // Put the array into the stream out.
