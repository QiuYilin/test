...
class NumGenerator {
 public:
  ...
  ~NumGenerator() { std::cout << "destruct now" << std::endl; }
};

struct getTwo {
  getTwo() { std::cout << "construct gettwo" << std::endl; }
  int operator()(int a, NumGenerator b) { return a + b.getNum(); }
};

int getRef(int a, NumGenerator &b) { return a + b.getNum(); }
...
class Function<Ret(Args...)> {
  ...
  struct callable_derived : public callable_base {
    ...
    struct callable_base *copy() const { return new callable_derived<T>(f); }
  };
...
int main() {
  // basic function
  Function<int(int)> getNumber(getOne);
